...
class RepositoryNameSuggester:
    ...
    def suggest_names(cls, description: str, current_name: str = "", count: int = 3) -> List[str]:
        ...
        common_words = {'the', 'a', 'an', 'is', 'are', 'for', 'to', 'of', 'in', 'and', 'or', 'this', 'that'}
...
class RepositoryHealthScorer:
    ...
    def calculate_score(cls, repo_files: List[str]) -> Dict[str, Any]:
        ...
        checks = {
            ...
            'has_code_of_conduct': any('code_of_conduct' in f or 'code-of-conduct' in f for f in files_lower),
            'has_security': any('security' in f for f in files_lower),
            'has_ci': any('.github/workflows' in f or '.gitlab-ci' in f or 'jenkinsfile' in f for f in files_lower),
