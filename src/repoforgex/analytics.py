...
class RepositoryAnalytics:
    ...
    def add_repository(self, name: str, owner: str, private: bool,
                       template: Optional[str] = None, **metadata):
        """
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'most_used_template': max(by_template.items(), key=lambda x: x[1])[0] if by_template else None,
        }
    ...
    def _analyze_name_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'common_prefixes': dict(list(sorted(prefixes.items(), key=lambda x: x[1], reverse=True))[:5]),
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if template_stats.get('none', 0) > summary['total_repos'] * 0.3:
            recommendations.append(
                "💡 Many repositories created without templates. Consider using templates for consistency."
            )
    ...
    def get_trend_analysis(self, time_window_hours: int = 24) -> Dict[str, Any]:
        ...
        actual_timespan = (
            datetime.now() - 
            datetime.fromisoformat(min(r['created_at'] for r in recent_repos))
        ).total_seconds() / 3600
